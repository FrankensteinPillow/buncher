...
from sqlalchemy import Date, Table, and_, func, text
...
def _filter_by_date(query: Select, dates: Tuple[date, date]) -> Select:
    if dates:
        query = query.where(
            and_(dataset.c.date >= dates[0], dataset.c.date <= dates[1])
        )
    return query
...
def _select_agregate_columns(
    columns: List[str], group_columns: Optional[List[str]]
) -> List[Column]:
    if group_columns is None:
        return []
    return _selected_columns(list(set(columns) & NUMERIC_COLS))
...
async def get_data(
    ...
    group_columns = _selected_columns(group_columns)
    columns = _selected_columns(columns)
    ...
    if "CPI" in selected_columns:
        ...
        query = select(
            *sbqry.columns, (sbqry.c.spend / sbqry.c.installs).label("CPI")
        )
    order: str = column_sort_by[0]
    if column_sort_by[1]:
        order = text(f"{column_sort_by[0]} {column_sort_by[1]}")
    query = query.order_by(order)
    sbqry = query.subquery("all_data")
